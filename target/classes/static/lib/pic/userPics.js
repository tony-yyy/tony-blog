let vm = new Vue({
    ...
    created: async function() { // 钩子函数，链式编程 es6新特性
        await axios.get("pictures/picturesDates/public")
            .then(response=>(
    ...
    methods: {
        ...
        loadImageByData: async function (datesIndexList) {
            ...
            for (var i = 0; i < datesIndexList.length; i++){
                ...
                // 根据日期获取当天所有图片对象
                await axios.get("pictures/getFilesByDate/public/" + date).then(function (response) {
        ...
        showFileDetail: function (index, pic) {
            ...
            // console.log(fileDetailDisplay.curIndex + ", " + fileDetailDisplay.pic)
        }
    },
})
