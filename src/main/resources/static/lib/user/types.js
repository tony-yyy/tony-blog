var blogs = new Vue({
    ...
    created: async function () {
        ...
        this.curType = this.types[0].id;
        this.getBlogs(1, this.curType, null);
    },
    methods: {
        getBlogs: async function(page, type, title){
            await axios.get(
                ...
                document.getElementById("waypoint").scrollIntoView();
            })
        },
        nextPage: function () {
            if (this.blogsPage.current + 1 > Math.ceil(this.blogsPage.total / this.blogsPage.size)) return;
            this.getBlogs(this.blogsPage.current + 1, this.curType, this.titleInput);
        },
        prePage: function () {
            if (this.blogsPage.current - 1 < 1) return;
            this.getBlogs(this.blogsPage.current - 1, this.curType, this.titleInput);
        },
        getBlogsByType: function (id) {
            this.curType = id;
            this.getBlogs(1, this.curType, null);
        }
    }
